			...
			if(jQuery.isEmptyObject(data)){
				...
				$.each(data,function(){
					if(this.arming_status==1&&this.online==0){
						$("#body").append("<div id=\"device_arming_sector\"><div id=\"changeColor\"><div id=\"device_info\" ><div><p>编号："+this.sec_id+"</p><p>名称："+this.name+"</p></div></div><div id=\"device_operate\"><p><img src=\"../image/cuo.png\"><button id=\"but1\">掉线</button></p><p><img src=\"../image/disarming.png\"><button id=\"but2\">撤防</button></p><p onclick=\"ShowDiv('"+this.id+"','"+this.sec_id+"','"+this.maxc_id+"','"+this.sub_id+"','MyDiv','fade')\"><img src=\"../image/operate2.png\"><button id=\"but3\">撤防</button></p></div></div></div>");
					}else if(this.arming_status==1&&this.online==1){
						$("#body").append("<div id=\"device_arming_sector\"><div id=\"changeColor\"><div id=\"device_info\" ><div><p>编号："+this.sec_id+"</p><p>名称："+this.name+"</p></div></div><div id=\"device_operate\"><p><img src=\"../image/dui.png\"><button id=\"but1\">在线</button></p><p><img src=\"../image/disarming.png\"><button id=\"but2\">撤防</button></p><p onclick=\"ShowDiv('"+this.id+"','"+this.sec_id+"','"+this.maxc_id+"','"+this.sub_id+"','MyDiv','fade')\"><img src=\"../image/operate2.png\"><button id=\"but3\">撤防</button></p></div></div></div>");
					}else if(this.arming_status==0&&this.online==0){
						$("#body").append("<div id=\"device_arming_sector\"><div id=\"changeColor\"><div id=\"device_info\" ><div><p>编号："+this.sec_id+"</p><p>名称："+this.name+"</p></div></div><div id=\"device_operate\"><p><img src=\"../image/cuo.png\"><button id=\"but1\">掉线</button></p><p><img src=\"../image/arming.png\"><button id=\"but2\">布防</button></p><p onclick=\"ShowDiv('"+this.id+"','"+this.sec_id+"','"+this.maxc_id+"','"+this.sub_id+"','MyDiv','fade')\"><img src=\"../image/operate2.png\"><button id=\"but3\">撤防</button></p></div></div></div>");
					}else if(this.arming_status==0&&this.online==1){
						$("#body").append("<div id=\"device_arming_sector\"><div id=\"changeColor\"><div id=\"device_info\" ><div><p>编号："+this.sec_id+"</p><p>名称："+this.name+"</p></div></div><div id=\"device_operate\"><p><img src=\"../image/dui.png\"><button id=\"but1\">在线</button></p><p><img src=\"../image/arming.png\"><button id=\"but2\">布防</button></p><p onclick=\"ShowDiv('"+this.id+"','"+this.sec_id+"','"+this.maxc_id+"','"+this.sub_id+"','MyDiv','fade')\"><img src=\"../image/operate2.png\"><button id=\"but3\">撤防</button></p></div></div></div>");
					}
		    	})
			}
		 }  
...
$(function() {
	...
	window.addEventListener('popstate', function(e) {
		...
		window.location.href = "/weixin/operate/broken.jsp?openId="+getParam("openId")+"&id="+maxc_id+"&number="+Math.random();
	}, false);
})
		...
		if(getParam("password")!=password){
			...
			if(flag){
				...
				$.ajax({  
					...
					url:"/weixin/servlet/sector_disArming.action",
					complete:function(){
						flag = true;
					}
				});
...
function getPassword(id,fPassword){
	if(flag){
		...
		$.ajax({  
			...
			success:function(data){
				if(data.remember!=null){ //判断是否有一键操作密码
					if(data.remember=="true"){//判断是否设置记住一键操作密码
