		...
		success:function(data){ 
			if(jQuery.isEmptyObject(data)){
				...
				$.each(data,function(){
					...
					$("#body").append("<div id=\"addDevice\" onclick=\"ShowDiv('MyDiv','fade')\"><p><img src=\"../image/add.png\"><button>一键布防</button></p></div>");
			    })
			}
		 }  
	});
...
$(function() {
	...
	window.addEventListener('popstate', function(e) {
	    //自定义操作//检查子页面有没有返回maxc_id
		 window.location.href = "/weixin/operate/device_arming.jsp?openId="+getParam("openId")+"&number="+Math.random();
	}, false);
})
...
function Arming(id,sub_id,MAXC_id){
	var openId = getParam("openId");
		...
		if(arr[3]!=password){
			...
			if(flag){
				...
				$.ajax({  
					...
					url:"/weixin/servlet/Arming.action",
					complete:function(){
						flag = true;
					}
				});
...
function findAllSector(id,sub_id,fPassword){
	var openId = getParam("openId");
	var zid = getParam("id");
	var url = "/weixin/operate/arming_sector.jsp?openId="+openId+"&id="+id+"&zid="+zid+"&sub_id="+sub_id+"&password="+fPassword;
	window.location=url;
}

function addSubOneArming(id,one_arming){
	if(flag){
		flag = false;
		...
		$.ajax({  
			dataType:"json", 
			type:"post", 
			data:{"id":id,"one_arming":one_arming,"openId":openId,"maxc_id":maxc_id,"name":name},
			url:"/weixin/servlet/addSubOneArming.action",
			complete:function(){
				flag = true;
			}
		});
		...
		if(onePassword!=""&&onePassword!=password){
			...
			if(flag){
				...
				$.ajax({  
					...
					url:"/weixin/servlet/oneSubArming.action",
					complete:function(){
						flag = true;
					}
				});
