...
function findSubDevice(id,openId,name){
	var url = "/weixin/operate/arming.jsp?id="+id+"&openId="+openId+"&name="+name;
	window.location=url;
}
...
function addOneArming(id){
	if(flag){
		...
		$.ajax({  
			dataType:"json", 
			type:"post", 
			data:{"id":id,"one_arming":one_arming,"openId":open},
			url:"/weixin/servlet/addOneArming.action",
			complete:function(){
				flag = true;
			}
		});
...
function oneArming(){
	...
	if(password==""){
		...
		if(onePassword!=""&&onePassword!=password){
			...
			if(flag){
				var remem = "not";//用来判断添加或修改一键操作密码的记住情况
				...
				$.ajax({  
					...
					url:"/weixin/servlet/oneArming.action",
					complete:function(){
						flag = true;
					}
				});
